...
Build.prototype.runTests = function(callBack) {
  ...
  exec('node ' + this.pathsConfig.testRunner + ' ci --launch=Chrome ' +
    this.pathsConfig.testHarness, options,
...
/**
 * Cleans up the distribution folder in preparation for
 * new and updated files.
 * @param {function=} callBack to execute
 */
Build.prototype.cleanDistDir = function(callBack) {
  var self = this;

  // Images...
  this.emit('log', 'Cleaning up images directory in dist... ');

  fs.rmrf(path.resolve(this.PATH_APPLAND_HOME,
    this.pathsConfig.assetsDistDir, './css/img'), function (err) {
    if (err) {
      self.emit('warn', 'Cannot remove images directory in dist.');
    }

    // Fonts...
    self.emit('log', 'Cleaning up fonts directory in dist... ');

    fs.rmrf(path.resolve(self.PATH_APPLAND_HOME,
      self.pathsConfig.assetsDistDir, './fonts'), function (err) {
      if (err) {
        self.emit('warn', 'Cannot remove fonts directory in dist.');
      }

      // CSS...
      self.emit('log', 'Removing CSS file in dist... ');

      fs.unlink(path.resolve(self.PATH_APPLAND_HOME,
        self.pathsConfig.assetsDistDir,
        './css/' + 'module-' + self.moduleName + '.css'), function (err) {
        if (err) {
          self.emit('warn', 'Cannot remove CSS file in dist.');
        }
