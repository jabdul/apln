...
function Create() {
  ...
  this.nameSpace = '';
  /**
   * Module's namespace UpperCamelCase version
   * @type {string}
   */
  this.nameSpaceCamel = '';
  /**
...
Create.prototype.setNameSpace = function(namespace) {
  if (this.validate('nameSpace', namespace)) {
    ...
    this.setNameSpaceCamel();
    return true;
  }
  return false;
};
/**
 * Manipulates Namespace to UpperCamelCase form.
 */
Create.prototype.setNameSpaceCamel = function() {
  if (!this.nameSpace) {
   return;
  }
  var parts = this.nameSpace.split('-');

  this.nameSpaceCamel = _(parts).map(function(part) {
...
Create.prototype.searchReplace = function( dest, source) {
  ...
  setTimeout(function() {
    var s = fs.createReadStream(source)
      ...
      .pipe(replaceStream('{MODULE-NAMESPACE-CAMEL}', that.nameSpaceCamel, null))
