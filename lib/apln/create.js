...
Create.prototype.compileCss = function(callBack) {
  var sassConfigDir = path.join(this.PATH_APPLAND_HOME, './src/assets'),
      ...
      sassModule = path.join(
        sassConfigDir, './sass/module-' + this.moduleName + '.scss'
      );
  ...
  compass.stderr.on('data', function(data) {
    that.emit('warn', 'Error ' + data + sassModule);
  });
  compass.on('exit', function(code, signal) {
    if (code == 0) {
      callBack();
    } else if (signal) {
      that.emit('warn', signal);
    }/* else {
      that.emit('log', signal);
    }*/
  });
};
/**
